...
import { naiveBayesClassifier } from './services/naiveBayesClassifier';
...
export default function TicketTriageInterface({ onBack }) {
  ...
  const [useMLClassification, setUseMLClassification] = useState(false);
  ...
  const checkModelStatus = () => {
    const stats = naiveBayesClassifier.getModelStats();
    if (stats.isTrained) {
      setModelStats(stats);
    }
  };
  ...
  const trainModel = async () => {
    setIsTraining(true);
    try {
      const result = await naiveBayesClassifier.train();
      if (result.success) {
        setModelStats(naiveBayesClassifier.getModelStats());
        alert(`Model trained successfully!\n\nTotal documents: ${result.totalDocs}\nVocabulary size: ${result.vocabularySize}\n\nClass distribution:\n${Object.entries(result.classDistribution).map(([cls, count]) => `${cls}: ${count}`).join('\n')}`);
      } else {
  ...
  const handleEmployeeSelection = (employeeName) => {
    if (employeeName) {
      setSelectedIssuer(employeeName);
      setForm(prev => ({ ...prev, issuerName: employeeName }));
  ...
  function classifyTicket(f) {
    // Use ML classification if enabled and model is trained
    if (useMLClassification && naiveBayesClassifier.isTrained) {
      try {
        const prediction = naiveBayesClassifier.predict({
          summary: f.description || '',
          description: f.description || '',
          issueType: f.ticketType || '',
          priority: f.severity || '',
          component: f.project || '',
        ...
        const actionMap = {
          ...
          'discard': 'Discard',
          'forward_other_team': 'Forward'
        };

        const reasons = [`ML Prediction: ${prediction.predictedClass} (${(prediction.confidence * 100).toFixed(1)}% confidence)`];
        
        if (prediction.predictedClass === 'need_more_info') {
          const infoIssues = needsMoreInfo(f);
          if (infoIssues.length > 0) {
            reasons.push(...infoIssues);
          }
        }

        return {
          action: actionMap[prediction.predictedClass] || 'Forward',
          reasons: reasons,
          forwardTo: prediction.predictedClass === 'forward_other_team' ? 'Triage' : null,
          discardReason: prediction.predictedClass === 'discard' ? 'ML model classified as discard' : null,
          ...
          <div className="max-w-4xl mx-auto">
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <header className="mb-8 text-center">
                <h2 className="text-3xl font-semibold text-slate-800 mb-4">Select Issuer</h2>
                <p className="text-slate-600">Choose the employee who will be issuing the ticket</p>
              </header>

              <div className="space-y-4">
                <label className="block text-sm font-medium text-slate-700 mb-4">Select Employee</label>
                <select 
              ...
              {employees.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-500">No employees found. Please check the salary service configuration.</p>
                </div>
              )}
            </div>
          </div>
              ...
              {selectedIssuer && (
                <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-green-800">Issuer: {selectedIssuer}</span>
                    <button
                      onClick={() => {
                        setShowEmployeeSelection(true);
                        setSelectedIssuer('');
                        setForm(prev => ({ ...prev, issuerName: '' }));
                      }}
                  ...
                  <div className="p-6 rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white">
                    ...
                    {result.mlPrediction && (
                      <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <h4 className="font-medium text-blue-800 mb-2">ML Prediction Details</h4>
                        <div className="text-sm text-blue-700 space-y-1">
                          <div>Confidence: {(result.mlPrediction.confidence * 100).toFixed(1)}%</div>
                          <div className="mt-2">
                            <span className="font-medium">Class Probabilities:</span>
                            <div className="mt-1 space-y-1">
                              {Object.entries(result.mlPrediction.probabilities).map(([cls, prob]) => (
                    ...
                    <details className="mt-6 text-sm text-slate-500">
                      ...
                      <div className="mt-3 p-4 bg-slate-50 rounded-lg">
                        <p className="text-sm leading-relaxed">
                          {useMLClassification && modelStats?.isTrained ? (
                            <>
                              <strong>ML Mode (Naive Bayes):</strong> The system uses a semi-supervised Naive Bayes classifier trained on {modelStats.totalDocs} historical tickets. 
                              It analyzes text features from the ticket description, type, priority, and other metadata to predict the most likely classification. 
                              The model provides confidence scores and class probabilities for transparency. If ML prediction fails, it falls back to rule-based classification.
                            </>
                          ) : (
                            <>
                              <strong>Rule-based Mode:</strong> This interface uses deterministic rules: it first validates whether critical fields are present, 
                              ...
                              and editable in the source code. Enable ML classification above to use the trained Naive Bayes model.
                            </>
                          )}
                        </p>
                      </div>
